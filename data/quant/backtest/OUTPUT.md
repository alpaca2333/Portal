...
| `bench_ret_{name}` | float | ⬜ | 基准当期收益率，`{name}` 为基准代码如 `000001.SH`（有基准时自动生成） |
| `excess_{name}` | float | ⬜ | 超额收益率 = `port_ret - bench_ret_{name}`（有基准时自动生成） |

**示例（含双基准）：**

```csv
date,port_ret,n_stocks,bench_ret_000001.SH,excess_000001.SH,bench_ret_000905.SZ,excess_000905.SZ
...
```csv
date,port_ret,n_stocks,bench_ret_000905.SZ,excess_000905.SZ
