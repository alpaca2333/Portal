...
def print_summary(
    ...
    print("=" * 64)
    print(f"策略名称 : {cfg.name}")
    print(f"回测区间 : {cfg.backtest_start} ~ {cfg.end}")
    print(f"调仓频率 : {cfg.freq.value}")
    print(f"选股规则 : 前 {cfg.top_pct:.0%}，每行业最多 {cfg.max_per_industry} 只")
    if cfg.buffer_sigma > 0:
        print(f"缓冲带   : 持仓 +{cfg.buffer_sigma}σ 加分")
    factor_str = ", ".join(f"{f.column}={f.weight:+.2f}" for f in factors)
    print(f"因子列表 : {factor_str}")
    bench_str = " / ".join(b.name for b in cfg.benchmarks[:2])
    print(f"基准指数 : {bench_str}")
    print("=" * 64)

    # Header
    headers = ["策略"] + [b.name.split("(")[0].strip() for b in cfg.benchmarks[:2]]
    hdr = f"{'指标':<20}" + "".join(f"{h:>14}" for h in headers)
    print(hdr)
    print("-" * len(hdr))
    ...
    print(f"{'胜率':<20}{port['win_rate']:>14.1%}" + "".join(f"{'—':>14}" for _ in bench_metrics))
    print(f"{'累计收益':<20}{port['cum_ret']:>+14.1%}" + "".join(f"{bm['cum_ret']:>+14.1%}" for bm in bench_metrics))

    for i, (em, ir) in enumerate(zip(excess_metrics, ir_vals)):
        bname = bench_names[i].split("(")[0].strip()
        if em:
            print(f"{'超额vs' + bname:<20}{em['ann_ret']:>+14.1%}")
...
def save_outputs(
    ...
    print(f"[保存] {nav_path.name}")
    print(f"[保存] {ret_path.name}")
...
def write_report(
    ...
    with open(path, "w", encoding="utf-8") as f:
        # Title
        f.write(f"# {cfg.name} 回测报告\n\n")
        f.write(f"**策略描述**: {cfg.description}  \n")
        if cfg.rationale:
            f.write("\n## 策略背景与改动原因\n\n")
            f.write(cfg.rationale.strip())
            f.write("\n\n")
        f.write(f"**回测区间**: {cfg.backtest_start} ~ {cfg.end}  \n")
        f.write(f"**调仓频率**: {cfg.freq.value}  \n")
        bench_str = " / ".join(b.name for b in cfg.benchmarks[:2])
        f.write(f"**基准指数**: {bench_str}  \n")
        f.write(f"**选股范围**: 沪深主板, 自由流通市值前 {cfg.mcap_keep_pct:.0%}  \n")
        f.write(f"**选股规则**: 前 {cfg.top_pct:.0%}，每行业最多 {cfg.max_per_industry} 只，等权  \n")
        f.write(f"**平均持仓**: {avg_n:.0f} 只 / {avg_ind:.0f} 个行业  \n")
        if cfg.buffer_sigma > 0:
            f.write(f"**缓冲带**: 持仓 +{cfg.buffer_sigma}σ 加分  \n")
        f.write(f"**交易成本**: 单边 {cfg.single_side_cost:.4%}  \n\n")
        ...
        f.write("|------|------|------|\n")
        for fac in factors:
            direction = "正向" if fac.weight > 0 else "反向"
        ...
        for bd, _, em, ir in bench_data:
            ...
            if em:
                f.write(f"| 超额vs{bname} | {em['ann_ret']:+.2%} | — | — |\n")
        ...
        # Transaction cost
        f.write("## 交易成本\n\n")
        f.write(f"- **单边费率**: {cfg.single_side_cost:.4%}\n")
        f.write(f"- **平均单边换手**: {avg_to:.1%}\n")
        f.write(f"- **累计成本拖累**: {total_tc:.2%}\n\n")
        if not np.isnan(gross.get("ann_ret", float("nan"))):
            f.write("| 指标 | 毛收益 | 净收益 | 差值 |\n")
            f.write("|------|--------|--------|------|\n")
            f.write(f"| 年化收益 | {gross['ann_ret']:+.2%} | {port['ann_ret']:+.2%} | {gross['ann_ret']-port['ann_ret']:.2%} |\n")
            f.write(f"| 夏普比率 | {gross['sharpe']:.2f} | {port['sharpe']:.2f} | {gross['sharpe']-port['sharpe']:.2f} |\n")
            f.write(f"| 累计收益 | {gross['cum_ret']:+.2%} | {port['cum_ret']:+.2%} | {gross['cum_ret']-port['cum_ret']:.2%} |\n")
        f.write("\n")
