"""
Download daily kline data from tushare.
Saves to: data/quant/data/daily/{ts_code}.csv   (one file per stock)

tushare API: pro.daily()
Fields saved: ts_code, trade_date, open, high, low, close, pre_close, change, pct_chg, vol, amount
"""
import os
import sys
import time
...
# ─── args ────────────────────────────────────────────────────────────
parser = argparse.ArgumentParser(description="下载日线行情数据")
...
# Thread-local storage for pro_api instances
...
def download_one(ts_code):
    """Download daily data for a single stock. Returns (ts_code, success, msg)."""
...
# ─── download ────────────────────────────────────────────────────────
print(f"正在下载日线数据 (daily)，共 {total} 只股票，起始日期: {START_DATE}，并发: {NUM_WORKERS} ...")

failed = []
with ThreadPoolExecutor(max_workers=NUM_WORKERS) as executor:
    futures = {executor.submit(download_one, code): code for code in ts_codes}
    tqdm_kwargs = dict(total=total, desc="日线数据", unit="只", ncols=80)
