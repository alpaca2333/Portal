"""
LightGBM Ensemble Adaptive Strategy V7a2
=========================================

V7a2 inherits all V7 risk controls and adds **4 oversold/reversal factors**
to both Smart Money (Model A) and Cross-Sectional (Model B) models, targeting
the mean-reversion alpha that V7 only captured weakly through `rev_10` and
`bottom_deviation`.
...
|--------|-------------|-----------|
| `rsi_14` | 14-day RSI (rank: lower is more oversold) | Direct oversold signal; rank-normalized so model sees relative cheapness |
| `ma60_dev` | (close / MA60) - 1, negated (rank: more negative = bigger discount) | Measures distance below MA60; complements `bottom_deviation` with medium-term anchor |
| `bounce_from_low` | (close / 20d-low) - 1 | Confirms price has started recovering from a trough (right-side confirmation) |
| `vol_surge_bottom` | 5d-avg-vol / 60d-avg-vol in last 5 days when price < MA60 | Detects capitulation volume at bottom; active only in oversold zone |

### Detailed Factor Attribution

`get_factor_exposures` now returns **per-model feature importance weights**
(LightGBM gain-based, normalized to sum=1 within each model) alongside the
existing market-state and industry-momentum columns. Output columns:

- `sm_{factor}` — Smart Money model gain-weight for each factor (25 factors)
- `cs_{factor}` — Cross-Sectional model gain-weight for each factor (25 factors)
- `model_a_score`, `model_b_score` — rank-normalized model scores
- `industry_momentum`, `small_cap`, `market_state` — existing composite signals

### V7 baseline (fully inherited)
...
# ===================================================================
# V7a2: Extended feature names (original + 3 V5 factors + 4 reversal factors)
# ===================================================================
V5_NEW_FACTORS = ["ivol", "max_ret_20d", "skew_20d"]
V7A2_REVERSAL_FACTORS = ["rsi_14", "ma60_dev", "bounce_from_low", "vol_surge_bottom"]
ALL_NEW_FACTORS = V5_NEW_FACTORS + V7A2_REVERSAL_FACTORS

SM_FEATURE_NAMES_V5 = list(SM_FEATURE_NAMES) + ALL_NEW_FACTORS
CS_FEATURE_NAMES_V5 = list(CS_FEATURE_NAMES) + ALL_NEW_FACTORS
...
def _compute_v5_factors(
    ...
    """
    Compute the 3 original V5 factors + 4 V7a2 reversal/oversold factors.

    V5 factors (unchanged):
      ivol        — idiosyncratic volatility (20d)
      max_ret_20d — maximum single-day return (20d lottery factor)
      skew_20d    — return skewness (20d)

    V7a2 new reversal factors:
      rsi_14           — 14-day RSI, negated so higher rank = more oversold
      ma60_dev         — (close/MA60)-1, negated so higher rank = bigger discount
      bounce_from_low  — (close/20d-low)-1; higher = more recovery from trough
      vol_surge_bottom — 5d vol / 60d vol, only active when price < MA60;
                         captures capitulation volume at bottom
    """
    ...
    if len(valid_dates) < 60:
        return {f: pd.Series(dtype=float) for f in ALL_NEW_FACTORS}

    # Get last 80 dates for factor computation (need 60 for MA60)
    window_dates = valid_dates[-80:]
    row_indices = []
    ...
    vol_pivot = window.pivot_table(
        index="trade_date", columns="ts_code", values="vol"
    ).sort_index() if "vol" in window.columns else None
    ...
    # ── V5 Factor 2: max_ret_20d ──────────────────────────────────────────
    if n_dates >= 21:
        factors["max_ret_20d"] = daily_ret.iloc[-20:].max()
    else:
        factors["max_ret_20d"] = pd.Series(dtype=float)

    # ── V5 Factor 3: skew_20d ─────────────────────────────────────────────
    if n_dates >= 21:
        factors["skew_20d"] = daily_ret.iloc[-20:].skew()
    else:
        factors["skew_20d"] = pd.Series(dtype=float)

    # ── V7a2 Factor 4: rsi_14 (rank-normalized: lower RSI = higher rank) ──
    # RSI < 30 → deeply oversold; we negate so that oversold = high feature value
    if n_dates >= 16:
        # Use last 15 returns for RSI(14)
        ret_rsi = daily_ret.iloc[-15:]
        gain = ret_rsi.clip(lower=0)
        loss = (-ret_rsi).clip(lower=0)
        ...
        # Negate: more oversold (lower RSI) → larger value → higher rank
        factors["rsi_14"] = -rsi
    else:
        factors["rsi_14"] = pd.Series(dtype=float)

    # ── V7a2 Factor 5: ma60_dev (negated) ────────────────────────────────
    # Higher rank → price further below MA60 → more attractive for mean reversion
    if n_dates >= 60:
        last_close = close_pivot.iloc[-1]
        ma60 = close_pivot.iloc[-60:].mean()
        dev = (last_close / ma60 - 1.0).replace([np.inf, -np.inf], np.nan)
        # Negate: bigger negative deviation → oversold → higher feature value
        factors["ma60_dev"] = -dev
    else:
        factors["ma60_dev"] = pd.Series(dtype=float)

    # ── V7a2 Factor 6: bounce_from_low ───────────────────────────────────
    # Right-side confirmation: has price bounced from recent trough?
    if n_dates >= 20:
        last_close = close_pivot.iloc[-1]
        low_20 = close_pivot.iloc[-20:].min()
        bounce = (last_close / low_20 - 1.0).replace([np.inf, -np.inf], np.nan)
        factors["bounce_from_low"] = bounce
    else:
        factors["bounce_from_low"] = pd.Series(dtype=float)

    # ── V7a2 Factor 7: vol_surge_bottom ──────────────────────────────────
    # Volume surge only when price is below MA60 (capitulation signal)
    if n_dates >= 60 and vol_pivot is not None and len(vol_pivot) >= 60:
        last_close = close_pivot.iloc[-1]
        ma60_v = close_pivot.iloc[-60:].mean()
        below_ma60 = (last_close < ma60_v)

        # vol_pivot aligned to close_pivot dates
        vp_aligned = vol_pivot.reindex(close_pivot.index).ffill()
        vol_5d = vp_aligned.iloc[-5:].mean()
        vol_60d = vp_aligned.iloc[-60:].mean()
        surge = (vol_5d / vol_60d.replace(0, np.nan)).replace([np.inf, -np.inf], np.nan)

        # Only meaningful when below MA60; set to 0 (neutral) otherwise
        surge_bottom = surge.where(below_ma60, other=0.0)
        factors["vol_surge_bottom"] = surge_bottom
    else:
        factors["vol_surge_bottom"] = pd.Series(dtype=float)
...
def sm_compute_features_v5(
    ...
    """Wrapper: compute original SM features + 7 extended factors (3 V5 + 4 V7a2)."""
...
def cs_compute_features_v5(
    ...
    """Wrapper: compute original CS features + 7 extended factors (3 V5 + 4 V7a2)."""
...
def sm_train_model_v5(
    ...
    """Train SM model with V7a2 extended features (22 + 7 = 29 factors)."""
    import lightgbm as lgb
    params = {
...
def cs_train_model_v5(
    ...
    """Train CS model with V7a2 extended features (22 + 7 = 29 factors)."""
    import lightgbm as lgb
    params = {
        "objective": "regression",
        "metric": "rmse",
...
class LGBMEnsembleAdaptiveV7a2(StrategyBase):
    """
    ...
    def describe(self) -> str:
        return (
            f"### 策略思路\n\n"
            f"自适应集成选股 **V7a2**（超跌反转增强版）：在V7风控框架基础上，"
            f"向两个子模型各增加4个超跌/反转因子，弥补V7对均值回归alpha捕捉不足的问题。\n\n"
            f"### V7a2 新增因子（4个超跌/反转因子）\n\n"
            f"| 因子 | 说明 | 逻辑 |\n"
            f"|------|------|------|\n"
            f"| `rsi_14` | 14日RSI取反 | RSI越低越超卖，取反后高排名=超卖 |\n"
            f"| `ma60_dev` | 价格偏离MA60（取反） | 价格折价越深，均值回归空间越大 |\n"
            f"| `bounce_from_low` | 价格从20日低点反弹幅度 | 右侧信号，底部已过才进场 |\n"
            f"| `vol_surge_bottom` | 价格低于MA60时的5日/60日成交量比 | 底部放量=资金接盘确认 |\n\n"
            f"### 详细因子归因（per-period）\n\n"
            f"每期 `factor_contrib.csv` 输出每只持仓股票在两个子模型的完整因子权重：\n"
            f"- `sm_*` 列：Smart Money 模型各因子的 gain 重要性（归一化至和=1）\n"
            f"- `cs_*` 列：Cross-Sectional 模型各因子的 gain 重要性（归一化至和=1）\n"
            f"- 每期更新（随模型重训而变化），可追踪超跌因子的权重演化\n\n"
            f"### V7 继承项（风控层，全部保留）\n\n"
            f"- 现金仓位管理（coeff控制实际资金比例）\n"
            f"- 组合级回撤熔断（>{abs(self.drawdown_circuit_breaker)*100:.0f}%触发，"
            f"缩减至{self.drawdown_reduction_factor*100:.0f}%）\n"
            f"- 最小持仓期（{self.min_holding_periods}期={self.min_holding_periods*2}周）\n"
            f"- 增强换手缓冲（buffer_sigma={self.buffer_sigma}）\n"
            f"- 收紧个股止损（{abs(self.stop_loss_threshold)*100:.0f}%）\n\n"
            f"### V5 继承项（因子层+模型层）\n\n"
            f"- 原始3个行为Alpha因子：ivol、max_ret_20d、skew_20d\n"
            f"- 行业动量评分、行业中性标签、双模型融合\n"
            f"- Smart Money ({self.weight_model_a}) + "
            f"Cross-Sectional ({self.weight_model_b})\n"
            f"- 市场状态五档仓位 + softmax加权(temperature={self.softmax_temperature})\n"
        )
    ...
    def get_factor_exposures(
        ...
        """
        Return per-stock factor exposures with detailed per-model feature importance.

        Output columns per stock:
        - ts_code, weight
        - model_a_score, model_b_score  — rank-normalized composite scores
        - small_cap, industry_momentum, market_state  — portfolio-level signals
        - sm_{factor}  — Smart Money model gain-based importance (normalized, per-period)
        - cs_{factor}  — Cross-Sectional model gain-based importance (normalized, per-period)

        Feature importance is gain-based (LightGBM), normalized so sum=1 within each model.
        The same importance applies to all selected stocks in a period (model-level, not stock-level),
        but is broadcast to each row for compatibility with the factor_contrib CSV schema.
        """
        if self._last_factor_exposures is None:
            return None

        df = self._last_factor_exposures.copy()

        # Attach per-model feature importance weights
        def _get_importance(model, feature_names, prefix):
            if model is None:
                return {}
            try:
                ...
                if total <= 0:
                    total = 1.0
                return {f"{prefix}{name}": float(imp[i] / total)
                        for i, name in enumerate(feature_names)}
            except Exception:
                return {}

        sm_imp = _get_importance(self._model_a, SM_FEATURE_NAMES_V5, "sm_")
        cs_imp = _get_importance(self._model_b, CS_FEATURE_NAMES_V5, "cs_")

        for col, val in sm_imp.items():
            df[col] = val
        for col, val in cs_imp.items():
            df[col] = val
    ...
    def generate_target_weights(
        ...
        print(f"\n      -- V7a2集成策略第 {self._call_count} 期  {date_str} --")
...
if __name__ == "__main__":
    ...
    strategy = LGBMEnsembleAdaptiveV7a2(
        # Model parameters
