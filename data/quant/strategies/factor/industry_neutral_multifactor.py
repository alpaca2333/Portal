...
# Transaction cost: single-side 1.5 bps (0.015%)
SINGLE_SIDE_COST = 0.00015  # 万分之1.5
...
def run_backtest(monthly):
    print("[backtest] Running monthly rebalance (with transaction cost) ...")
    periods = sorted(monthly["ym"].unique())
    bt_start = pd.Period(BACKTEST_START[:7], "M")
    results = []
    prev_holdings = set()  # track previous month's holdings for turnover calc
    ...
    for i in range(len(periods)-1):
        ...
        # Turnover calculation (equal weight assumed)
        # turnover = fraction of portfolio that changed
        if len(prev_holdings) == 0:
            # First month: buy everything from cash → 100% buy turnover
            turnover_buy = 1.0
            turnover_sell = 0.0
        else:
            ...
            # Sell weight = fraction of old portfolio sold
            turnover_sell = len(sold) / n_prev if n_prev > 0 else 0.0
            # Buy weight = fraction of new portfolio that is new
            turnover_buy = len(bought) / n_curr if n_curr > 0 else 0.0

        # Transaction cost = single_side_cost × (sell_turnover + buy_turnover)
    ...
    if not result_df.empty:
        ...
        print(f"[backtest] Avg monthly cost: {avg_tc:.4%}")
...
def write_report(df):
    ...
    with open(path, "w", encoding="utf-8") as f:
        ...
        f.write(f"| 最大回撤 | {gross_m['max_dd']:.2%} | {port['max_dd']:.2%} | — |\n")
...
if __name__ == "__main__":
    ...
    combined["excess_gross"] = combined["port_ret_gross"] - combined["bench_ret"]
    combined["date"] = pd.to_datetime(combined["date"]).dt.strftime("%Y-%m-%d")

    print("\n[8/8] 输出结果 ...")
    print_summary(combined.copy())

    # Also print gross vs net comparison
    gross_m = calc_return_metrics(combined["port_ret_gross"])
    net_m = calc_return_metrics(combined["port_ret"])
    ...
    print(f"平均单边月换手率：{avg_to:.1%}")
    print(f"平均月度交易成本：{avg_tc:.4%}")
    ...
    print(f"{'最大回撤':18} {gross_m['max_dd']:>12.2%} {net_m['max_dd']:>12.2%}")
    print("="*64)
    ...
    returns_df = combined[["date","port_ret","port_ret_gross","n_stocks",
