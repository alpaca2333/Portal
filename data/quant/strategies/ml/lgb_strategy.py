...
HOLDOUT_MONTHS = 12               # 留出最后 n 个月作为纯样本外（真正的OOS验证集）
                                  # 注意：不应超过 ~18 个月，否则滚动训练覆盖范围严重不足。
                                  # 原值 86 个月导致仅 2 个滚动窗口，IC 不具代表性。
...
# ── 从 DATA_END 正向推导全部时间节点 ──
# 布局：
#   WARM_UP_START ... MIN_DATE(数据可用最早) ... 滚动窗口 ... MODEL_CUTOFF ... DATA_END
#
# MODEL_CUTOFF = DATA_END - HOLDOUT_MONTHS（留出期分隔线）
# MIN_DATE     = 固定为数据集实际可用最早日期（由数据决定，不从 MODEL_CUTOFF 反推）
#                这样无论 HOLDOUT_MONTHS 怎么设置，滚动窗口数量都不会受影响。
# WARM_UP_START = MIN_DATE - WARM_UP_YEARS
...
# 2. MIN_DATE：从数据实际可用起点正向推导（固定锚点，不依赖 HOLDOUT_MONTHS）
#    此处设为数据集中有足够股票的最早日期（约 2010 年起有沪深数据）
#    + WARM_UP_YEARS 预热期，确保特征有足够历史
_DATA_AVAILABLE_FROM = pd.Timestamp("2010-01-01")
_min_date_dt = _DATA_AVAILABLE_FROM + pd.DateOffset(years=_WARM_UP_YEARS)

# 3. WARM_UP_START = MIN_DATE - WARM_UP_YEARS（特征预热窗口起始）
