"""
LightGBM Ensemble Adaptive Strategy V8
=======================================

V8 introduces **Asymmetric Position Management** (不对称仓位管理) to solve
V7a's core problem: Cash-Aware sizing was too aggressive in cutting exposure,
reducing annual return from 27.3% (V7) to 5.8% (V7a) while only cutting
max drawdown from 32.5% to 9.1% — an unfavorable trade.

**V8 Core Innovation: Base + Timing Split**

Instead of scaling ALL capital by the market state coefficient, V8 splits
the portfolio into two components:

  Total Allocation = Base Allocation (fixed 70%) + Timing Allocation (0-30%)

- **Base Allocation (70%)**: Always fully invested, never affected by
  market state timing. Ensures a minimum return floor.
- **Timing Allocation (0-30%)**: Dynamically controlled by market state
  coefficient. In strong bull: full 30%. In panic: 0%.

Effective allocation by market state:
  Strong Bull:  70% + 30% × 1.0 = 100%  (full)
  Mild Bull:    70% + 30% × 0.8 = 94%
  Range-bound:  70% + 30% × 0.5 = 85%
  Weak Bear:    70% + 30% × 0.3 = 79%
  Panic:        70% + 30% × 0.0 = 70%   (minimum)

vs V7a's allocation:
  Strong Bull=100%, Mild Bull=80%, Range=50%, Weak Bear=30%, Panic=10%

**V8 Secondary: Bull Recovery Acceleration**

When market state transitions from lower to higher coefficient, the timing
allocation recovers 80% of the gap in one period instead of stepping through
discrete levels. This prevents missing the first wave of a bull market.

**V8 Tertiary: Differentiated Circuit Breaker**

Portfolio drawdown > 15%:
  - If benchmark also down > 10%: systemic risk → full reduction (50%)
  - If benchmark stable: idiosyncratic risk → light reduction (80%)

Inherited from V7/V7a:
- Virtual CASH position sizing (V7a bugfix)
...
- Dual LightGBM ensemble (Smart Money 0.6 + Cross-Sectional 0.4)
- Minimum holding period (2 periods), turnover buffer (sigma=1.0)
- Stop-loss with cooldown (-12%)
- Factor attribution interface

Key Parameter Changes (V7a → V8):
- base_allocation:      N/A → 0.70 (new: always-invested floor)
- timing_allocation:    N/A → 0.30 (new: market-state-driven portion)
- recovery_speed:       N/A → 0.80 (new: bull recovery acceleration)
- coeff mapping:        {1.0,0.8,0.5,0.3,0.1} → {1.0,0.8,0.5,0.3,0.0}
  (panic 0.1→0.0 because base_allocation already guarantees 70%)

Usage
-----
cd /data/Projects/Portal
python -m data.quant.strategies.lgbm_ensemble_adaptive_v8
"""
import sys
import os
import warnings
...
class LGBMEnsembleAdaptiveV8(StrategyBase):
    """
    ...
    def __init__(
        ...
        # V8: Asymmetric position management
        base_allocation: float = 0.70,       # always-invested floor
        timing_allocation: float = 0.30,     # market-state-driven portion
        recovery_speed: float = 0.80,        # bull recovery acceleration
        ...
        super().__init__("lgbm_ensemble_adaptive_v8")
        ...
        self.timing_allocation = timing_allocation
        ...
        # V8: Previous period's timing coefficient for recovery acceleration
        self._prev_timing_coeff: float = 0.5
    ...
    def describe(self) -> str:
        return (
            f"### 策略思路\n\n"
            f"自适应集成选股 V8（不对称仓位管理版）：解决V7a过度保守的核心问题。"
            f"V7a的Cash-Aware机制虽然将最大回撤控制在9.1%，但年化收益从27.3%暴跌到5.8%。"
            f"V8通过不对称仓位管理，在保持风控的同时大幅提升收益。\n\n"
            f"### V8 核心创新：不对称仓位管理\n\n"
            f"总仓位 = 基础仓位({self.base_allocation*100:.0f}%固定) + "
            f"择时仓位(0-{self.timing_allocation*100:.0f}%动态)\n\n"
            f"- **基础仓位({self.base_allocation*100:.0f}%)**：永远满仓运作，不受市场状态影响，"
            f"保证收益下限\n"
            f"- **择时仓位({self.timing_allocation*100:.0f}%)**：根据市场状态动态调整，"
            f"强牛满配、恐慌清零\n"
            f"- **实际仓位范围**：{self.base_allocation*100:.0f}%（恐慌）~ 100%（强牛）\n\n"
            f"### V8 次级改进\n\n"
            f"- **牛市恢复加速**：仓位恢复时一期内追回{self.recovery_speed*100:.0f}%的差距，"
            f"避免错过牛市第一波\n"
            f"- **差异化回撤熔断**：区分系统性风险（大盘同跌→正常减仓）和特质性风险"
            f"（大盘稳组合跌→轻度减仓，重点换股）\n\n"
            f"### 继承项（V7/V7a）\n\n"
            f"- 虚拟CASH占位实现现金仓位管理（V7a bugfix）\n"
            f"- 3个行为Alpha因子：ivol、max_ret_20d、skew_20d\n"
            f"- 连续化行业动量 + 行业中性标签\n"
            f"- 双模型融合：Smart Money ({self.weight_model_a}) + "
            f"Cross-Sectional ({self.weight_model_b})\n"
            f"- 最小持仓期({self.min_holding_periods}期) + 换手缓冲(sigma={self.buffer_sigma})\n"
            f"- 个股止损(-{abs(self.stop_loss_threshold)*100:.0f}%) + 组合熔断\n"
        )
    ...
    def _compute_market_state(self, close_pivot: pd.DataFrame) -> float:
        ...
        Returns coefficient in {0.0, 0.3, 0.5, 0.8, 1.0} (V8: panic=0.0).
        """
        n = len(close_pivot)
        if n < 60:
        ...
        if is_uptrend:
            ...
            if above_ratio >= 0.30:
                ...
                coeff = 0.00  # panic (V8: 0.0 because base_allocation guarantees 70%)
    ...
    def generate_target_weights(
        ...
        print(f"\n      -- V8集成策略第 {self._call_count} 期  {date_str} --")
        ...
        close_pivot = None  # V8: needed for differentiated circuit breaker
        if row_indices:
            all_row_idx = np.concatenate(row_indices)
        ...
        # V8: Asymmetric position management
        # Total allocation = base_allocation (fixed) + timing_allocation * timing_coeff
        timing_coeff = position_coeff  # raw market state coefficient

        # V8: Bull recovery acceleration
        # When timing_coeff is rising, recover 80% of the gap in one period
        if timing_coeff > self._prev_timing_coeff:
            gap = timing_coeff - self._prev_timing_coeff
            boost = gap * self.recovery_speed
            timing_coeff = min(1.0, self._prev_timing_coeff + gap + boost * 0.5)
            # Note: we boost by half the recovery_speed * gap on top of natural move
            # This means recovery is ~1.4x faster than natural step

        # V8: Differentiated circuit breaker
        if self._circuit_breaker_active:
            # Check if benchmark is also down (systemic) or just our portfolio (idiosyncratic)
            # Use market median as proxy for benchmark
            if close_pivot is not None and len(close_pivot) >= 60:
                market_median = close_pivot.median(axis=1)
                if len(market_median) >= 20:
                    bench_recent = market_median.iloc[-1]
                    bench_peak = market_median.iloc[-20:].max()
                    bench_dd = bench_recent / bench_peak - 1 if bench_peak > 0 else 0
                    if bench_dd < -0.10:
                        # Systemic risk: full reduction
                        cb_reduction = self.drawdown_reduction_factor  # 0.5
                    else:
                        # Idiosyncratic risk: light reduction
                        cb_reduction = 0.8
                else:
                    cb_reduction = self.drawdown_reduction_factor
            else:
                cb_reduction = self.drawdown_reduction_factor
            timing_coeff = timing_coeff * cb_reduction

        # Store for next period's recovery calculation
        self._prev_timing_coeff = timing_coeff

        # V8: Compute final allocation
        final_alloc = self.base_allocation + self.timing_allocation * timing_coeff
        final_alloc = min(1.0, max(self.base_allocation, final_alloc))

        # effective_max still based on final_alloc for position count
        effective_max = max(2, int(self.max_positions * final_alloc))
        self._cash_ratio = 1.0 - final_alloc

        state_labels = {
            1.0: "强牛", 0.8: "普牛", 0.5: "震荡", 0.3: "弱熊", 0.0: "恐慌"
        }
        state_name = state_labels.get(position_coeff, f"coeff={position_coeff}")
        breaker_str = " [熔断中]" if self._circuit_breaker_active else ""
        print(f"      [V8择时] 市场状态={state_name}  "
              f"基础仓={self.base_allocation:.0%} + 择时仓={self.timing_allocation:.0%}×{timing_coeff:.2f}  "
              f"总仓位={final_alloc:.0%}{breaker_str}  "
        ...
        # Step 10.7 (V8): Cash-aware position sizing via virtual CASH entry
        # Inject virtual "CASH" entry so broker sees sum=1.0 but holds cash.
        if weights and final_alloc < 1.0:
            cash_weight = 1.0 - final_alloc
            # Scale stock weights so stocks + CASH = 1.0
            stock_total = sum(weights.values())
            if stock_total > 0:
                scale = final_alloc / stock_total
                for code in weights:
                    weights[code] *= scale
            weights["CASH"] = cash_weight
            print(f"      [V8] 不对称仓位: 基础{self.base_allocation:.0%}+择时{self.timing_allocation*timing_coeff:.0%}  "
                  f"股票权重={final_alloc:.2f}, 现金占位={cash_weight:.2f}")
...
if __name__ == "__main__":
    ...
    strategy = LGBMEnsembleAdaptiveV8(
        # Model parameters
        ...
        # V8: Asymmetric position management
        base_allocation=0.70,       # 70% always invested
        timing_allocation=0.30,     # 30% market-state driven
        recovery_speed=0.80,        # recover 80% of gap per period
        # Stock pool parameters
        mv_pct_upper=0.85,
