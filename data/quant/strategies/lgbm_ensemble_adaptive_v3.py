"""
LightGBM Ensemble Adaptive Strategy V3
=======================================

Improvements over V2:
1. Small-cap exposure cap: dynamic suppression of small_cap_bonus when
   market volatility spikes, plus hard cap on total small-cap weight
2. Proactive risk signals: market breadth acceleration, limit-down ratio,
   volume shrinkage → composite risk_score for early position reduction

Inherited from V2:
- Volatility targeting replaces discrete 5-level position sizing
- Triple trend confirmation (short/mid/long)
- Adaptive stop-loss based on individual stock volatility
- Dynamic style factor (large/small cap adaptation)
- Training optimization: 4yr window, sample weighting, 6-period retrain
- Portfolio construction: dynamic softmax temperature, relaxed industry constraint

Usage
-----
cd D:\\Projects\\Portal
python -m data.quant.strategies.lgbm_ensemble_adaptive_v3
"""
import sys
import os
import warnings
...
class LGBMEnsembleAdaptiveV3(StrategyBase):
    """
    V3: Small-cap exposure cap + proactive risk signals.
    Built on V2 (volatility targeting + multi-level timing + adaptive stop-loss).
    """
    ...
    def __init__(
        ...
        # V3: Small-cap exposure cap
        small_cap_weight_cap: float = 0.30,
        small_cap_weight_cap_stress: float = 0.15,
        small_cap_stress_vol_threshold: float = 0.50,
        small_cap_mv_quantile: float = 0.30,
        small_cap_suppress_vol_threshold: float = 0.50,
        risk_small_cap_filter_threshold: float = 0.80,
        risk_small_cap_mv_pct: float = 0.25,
        # V3: Proactive risk signals
        risk_limit_down_threshold: float = 0.03,
        risk_breadth_accel_threshold: float = -0.08,
        risk_volume_shrink_threshold: float = 0.60,
        risk_composite_floor: float = 0.40,
    ):
        super().__init__("lgbm_ensemble_adaptive_v3")
        ...
        # V3: Small-cap exposure cap
        self.small_cap_weight_cap = small_cap_weight_cap
        self.small_cap_weight_cap_stress = small_cap_weight_cap_stress
        self.small_cap_stress_vol_threshold = small_cap_stress_vol_threshold
        self.small_cap_mv_quantile = small_cap_mv_quantile
        self.small_cap_suppress_vol_threshold = small_cap_suppress_vol_threshold
        self.risk_small_cap_filter_threshold = risk_small_cap_filter_threshold
        self.risk_small_cap_mv_pct = risk_small_cap_mv_pct

        # V3: Proactive risk signals
        self.risk_limit_down_threshold = risk_limit_down_threshold
        self.risk_breadth_accel_threshold = risk_breadth_accel_threshold
        self.risk_volume_shrink_threshold = risk_volume_shrink_threshold
        self.risk_composite_floor = risk_composite_floor

    def describe(self) -> str:
        return (
            f"### 策略思路\n\n"
            f"自适应集成选股 V3：在 V2 基础上新增小盘暴露上限和前瞻性风控信号。"
            f"核心选股层沿用 V2（双模型 44 因子 + 共识过滤）。\n\n"
            f"### V3 核心改进\n\n"
            f"1. **小盘暴露上限**：小盘股(市值后{self.small_cap_mv_quantile*100:.0f}%)总权重"
f"上限{self.small_cap_weight_cap*100:.0f}%(压力时{self.small_cap_weight_cap_stress*100:.0f}%)；"
            f"当 vol_scale < {self.small_cap_suppress_vol_threshold:.2f} 时自动压制 small_cap_bonus\n"
            f"2. **前瞻性风控信号**：\n"
            f"   - 市场宽度加速度（二阶导数 < {self.risk_breadth_accel_threshold}）\n"
            f"   - 跌停比率（> {self.risk_limit_down_threshold*100:.0f}% 触发预警）\n"
            f"   - 成交量萎缩（< {self.risk_volume_shrink_threshold*100:.0f}% 均量触发预警）\n"
            f"   - 综合风控评分直接调节仓位，下限 {self.risk_composite_floor*100:.0f}%\n"
            f"3. **风控驱动选股过滤**：当风控乘数 < {self.risk_small_cap_filter_threshold:.0%} 时，"
            f"直接从选股池剔除市值后{self.risk_small_cap_mv_pct*100:.0f}%的小盘股\n\n"
            f"### 继承自 V2\n\n"
            f"- 波动率目标制（目标 {self.target_vol*100:.0f}%）\n"
            f"- 三级趋势确认（短/中/长期）\n"
            f"- 自适应止损（{self._stop_loss.max_threshold*100:.0f}% ~ "
    ...
    # ── V3: Proactive risk signal methods ──

    def _compute_breadth_acceleration(self, close_pivot: pd.DataFrame) -> float:
        """
        V3: Breadth acceleration (2nd derivative).
        Measures how fast breadth is deteriorating.
        Negative = breadth worsening at increasing speed → danger signal.
        """
        if len(close_pivot) < 30:
            return 0.0
        stock_ma20 = close_pivot.rolling(20).mean()
        # Compute breadth time series for last 10 days
        breadth_ts = []
        for offset in range(10, 0, -1):
            idx = -offset
            if abs(idx) <= len(close_pivot) and abs(idx) <= len(stock_ma20):
                b = (close_pivot.iloc[idx] > stock_ma20.iloc[idx]).mean()
                breadth_ts.append(b)
        if len(breadth_ts) < 5:
            return 0.0
        # 1st derivative (velocity)
        velocity = [breadth_ts[i] - breadth_ts[i-1] for i in range(1, len(breadth_ts))]
        # 2nd derivative (acceleration) - average of last 3
        if len(velocity) < 3:
            return 0.0
        accel = [velocity[i] - velocity[i-1] for i in range(1, len(velocity))]
        return np.mean(accel[-3:])

    def _compute_limit_down_ratio(self, close_pivot: pd.DataFrame) -> float:
        """
        V3: Ratio of stocks hitting limit-down (daily return <= -9.5%)
        over the last 5 trading days. Spike = systemic panic.
        """
        if len(close_pivot) < 6:
            return 0.0
        daily_ret = close_pivot.pct_change()
        last_5 = daily_ret.iloc[-5:]
        # Count limit-down events (ret <= -9.5% as proxy for -10% limit)
        limit_down_count = (last_5 <= -0.095).sum().sum()
        total_observations = last_5.notna().sum().sum()
        if total_observations == 0:
            return 0.0
        return limit_down_count / total_observations

    def _compute_volume_shrink_ratio(
        self, window_data: pd.DataFrame
    ) -> float:
        """
        V3: Current 5-day average volume / 20-day average volume.
        < 1.0 means volume is shrinking; < 0.6 is a warning signal.
        """
        if window_data is None or window_data.empty:
            return 1.0
        if "amount" not in window_data.columns:
            return 1.0
        # Aggregate daily total market volume
        daily_vol = window_data.groupby("trade_date")["amount"].sum().sort_index()
        if len(daily_vol) < 20:
            return 1.0
        vol_5d = daily_vol.iloc[-5:].mean()
        vol_20d = daily_vol.iloc[-20:].mean()
        if vol_20d <= 0:
            return 1.0
    ...
    def _compute_risk_score(
        ...
        """
        V3: Composite proactive risk score.
        Returns (risk_multiplier, detail_dict).
        risk_multiplier in [risk_composite_floor, 1.0]:
          1.0 = no risk detected, full position
          risk_composite_floor = maximum risk, minimum position
        """
        details = {}

        # Signal 1: Breadth acceleration
        breadth_accel = self._compute_breadth_acceleration(close_pivot)
        details["breadth_accel"] = breadth_accel
        accel_penalty = 0.0
        if breadth_accel < self.risk_breadth_accel_threshold:
            # Scale penalty: the more negative, the worse
            excess = abs(breadth_accel) - abs(self.risk_breadth_accel_threshold)
            accel_penalty = min(0.25, excess * 5.0)  # max 25% penalty

        # Signal 2: Limit-down ratio
        limit_down_ratio = self._compute_limit_down_ratio(close_pivot)
        details["limit_down_ratio"] = limit_down_ratio
        limit_penalty = 0.0
        if limit_down_ratio > self.risk_limit_down_threshold:
            excess = limit_down_ratio - self.risk_limit_down_threshold
            limit_penalty = min(0.30, excess * 5.0)  # max 30% penalty

        # Signal 3: Volume shrinkage
        vol_ratio = self._compute_volume_shrink_ratio(window_data)
        details["volume_ratio"] = vol_ratio
        vol_penalty = 0.0
        if vol_ratio < self.risk_volume_shrink_threshold:
            deficit = self.risk_volume_shrink_threshold - vol_ratio
            vol_penalty = min(0.15, deficit * 0.5)  # max 15% penalty

        # Composite: multiplicative penalties
        total_penalty = accel_penalty + limit_penalty + vol_penalty
        risk_multiplier = max(self.risk_composite_floor, 1.0 - total_penalty)
        details["total_penalty"] = total_penalty
        details["risk_multiplier"] = risk_multiplier

        return risk_multiplier, details

    def _apply_small_cap_weight_cap(
        self,
        ...
        """
        V3: Enforce small-cap total weight cap.
        - Normal: cap = small_cap_weight_cap (default 30%)
        - Stress (vol_scale < threshold): cap = small_cap_weight_cap_stress (15%)
        If small-cap stocks exceed the cap, proportionally reduce their
        weights and redistribute to large-cap stocks.
        """
        if not weights or "circ_mv" not in feat_df.columns or "ts_code" not in feat_df.columns:
            return weights

        # Dynamic cap: tighten under high volatility
        if vol_scale < self.small_cap_stress_vol_threshold:
            effective_cap = self.small_cap_weight_cap_stress
        else:
            effective_cap = self.small_cap_weight_cap

        mv_map = feat_df.set_index("ts_code")["circ_mv"].to_dict()
        mv_values = [mv_map.get(c, np.nan) for c in weights.keys()]
        valid_mv = [v for v in mv_values if not np.isnan(v)]
        if len(valid_mv) < 5:
            return weights

        mv_threshold = np.quantile(valid_mv, self.small_cap_mv_quantile)

        small_codes = []
        large_codes = []
        for code in weights:
            mv = mv_map.get(code, np.nan)
            if np.isnan(mv) or mv <= mv_threshold:
                small_codes.append(code)
            else:
                large_codes.append(code)

        small_total = sum(weights[c] for c in small_codes)

        if small_total <= effective_cap or not large_codes:
            return weights

        # Need to reduce small-cap weight
        excess = small_total - effective_cap
        scale_factor = effective_cap / small_total

        new_weights = dict(weights)
        for code in small_codes:
            new_weights[code] = weights[code] * scale_factor

        # Redistribute excess to large-cap stocks proportionally
        large_total = sum(weights[c] for c in large_codes)
        if large_total > 0:
            for code in large_codes:
                new_weights[code] = weights[code] + excess * (weights[code] / large_total)
    ...
    def generate_target_weights(
        ...
        print(f"\n      -- V3集成策略第 {self._call_count} 期  {date_str} --")
        ...
        if row_indices:
            ...
            # V3: Proactive risk signals
            risk_multiplier, risk_details = self._compute_risk_score(
                close_pivot, window_data,
            )
            # V3-fix: Do NOT multiply into regime_modifier (which gets
            # swallowed by min(vol_scale, regime_modifier)).  Instead,
            # store risk_multiplier and apply it AFTER the min().
            self._current_risk_multiplier = risk_multiplier
            self._current_risk_details = risk_details
            if risk_multiplier < 0.95:
                print(
                    f"      [V3风控] 前瞻性风控触发: "
                    f"宽度加速={risk_details['breadth_accel']:.4f}  "
                    f"跌停比率={risk_details['limit_down_ratio']:.4f}  "
                    f"量比={risk_details['volume_ratio']:.2f}  "
                    f"→ 风控乘数={risk_multiplier:.2f}"
                )
            ...
            self._current_risk_details = {}

        # V2: Volatility targeting
        vol_scale = self._compute_vol_target_scale()
        self._prev_vol_scale = vol_scale

        # V3: Suppress small_cap_bonus when volatility is high
        if vol_scale < self.small_cap_suppress_vol_threshold:
            if small_cap_bonus > 0:
                print(
                    f"      [V3小盘] vol_scale={vol_scale:.2f} < "
                    f"{self.small_cap_suppress_vol_threshold:.2f}，"
                    f"压制小盘加分 {small_cap_bonus:.3f} → 0.000"
                )
                small_cap_bonus = 0.0

        # V2: Combined position sizing
        # V3-fix: Apply risk_multiplier AFTER the min(), so it always
        # takes effect even when vol_scale is already the binding constraint.
        base_scale = min(vol_scale, regime_modifier)
        final_scale = base_scale * risk_multiplier
        final_scale = max(final_scale, 0.10)  # absolute floor 10%
        effective_max = max(3, int(self.max_positions * final_scale))

        risk_info = ""
        if risk_multiplier < 0.95:
            risk_info = f"  风控乘数={risk_multiplier:.2f}"
        print(f"      [择时] 市场状态={market_regime}  "
              ...
              f"温度={regime_temperature:.1f}{risk_info}")
        ...
        # Step 9b (V3): When risk signals fire, remove small-cap stocks
        # from the candidate pool BEFORE selection, so they cannot be picked.
        if risk_multiplier < self.risk_small_cap_filter_threshold:
            if "circ_mv" in feat_a_ranked.columns:
                mv_map_filter = feat_a_ranked.set_index("ts_code")["circ_mv"].to_dict()
                mv_vals_filter = [v for v in mv_map_filter.values() if not np.isnan(v)]
                if len(mv_vals_filter) >= 20:
                    mv_cutoff = np.quantile(mv_vals_filter, self.risk_small_cap_mv_pct)
                    small_cap_codes = {
                        c for c in ensemble_scores.index
                        if mv_map_filter.get(c, np.nan) <= mv_cutoff
                        or np.isnan(mv_map_filter.get(c, np.nan))
                    }
                    if small_cap_codes:
                        n_before = len(ensemble_scores)
                        ensemble_scores = ensemble_scores.drop(
                            small_cap_codes & set(ensemble_scores.index),
                            errors="ignore",
                        )
                        # Also update ranks
                        rank_a = rank_a.reindex(ensemble_scores.index)
                        rank_b = rank_b.reindex(ensemble_scores.index)
                        n_removed = n_before - len(ensemble_scores)
                        print(
                            f"      [V3风控] 风控乘数={risk_multiplier:.2f} < "
                            f"{self.risk_small_cap_filter_threshold:.2f}，"
                            f"从选股池剔除小盘股 {n_removed} 只 "
                            f"(市值后{self.risk_small_cap_mv_pct*100:.0f}%)，"
                            f"剩余 {len(ensemble_scores)} 只"
                        )
                        if ensemble_scores.empty:
                            print(f"      [V3风控] [X] 剔除后无候选股")
                            return {}
        ...
        # Step 10a (V3): Enforce small-cap weight cap (dynamic by vol_scale)
        if weights:
            weights_before = dict(weights)
            weights = self._apply_small_cap_weight_cap(weights, feat_a_ranked, vol_scale)
            # Log if cap was applied
            small_cap_total_before = 0.0
            small_cap_total_after = 0.0
            if "circ_mv" in feat_a_ranked.columns:
                mv_map = feat_a_ranked.set_index("ts_code")["circ_mv"].to_dict()
                mv_vals = [mv_map.get(c, np.nan) for c in weights.keys()]
                valid_mv = [v for v in mv_vals if not np.isnan(v)]
                if len(valid_mv) >= 5:
                    mv_thresh = np.quantile(valid_mv, self.small_cap_mv_quantile)
                    for c in weights:
                        mv = mv_map.get(c, np.nan)
                        if np.isnan(mv) or mv <= mv_thresh:
                            small_cap_total_before += weights_before.get(c, 0)
                            small_cap_total_after += weights.get(c, 0)
                    eff_cap = self.small_cap_weight_cap_stress if vol_scale < self.small_cap_stress_vol_threshold else self.small_cap_weight_cap
                    if abs(small_cap_total_before - small_cap_total_after) > 0.01:
                        stress_tag = " [压力模式]" if vol_scale < self.small_cap_stress_vol_threshold else ""
                        print(
                            f"      [V3小盘] 小盘权重压缩: "
                            f"{small_cap_total_before:.1%} → {small_cap_total_after:.1%} "
                            f"(上限 {eff_cap:.0%}{stress_tag})"
                        )
...
if __name__ == "__main__":
    ...
    strategy = LGBMEnsembleAdaptiveV3(
        # Model parameters (V2)
        ...
        # V3: Small-cap exposure cap
        small_cap_weight_cap=0.30,
        small_cap_weight_cap_stress=0.15,
        small_cap_stress_vol_threshold=0.50,
        small_cap_mv_quantile=0.30,
        small_cap_suppress_vol_threshold=0.50,
        # V3: Proactive risk signals
        risk_limit_down_threshold=0.03,
        risk_breadth_accel_threshold=-0.08,
        risk_volume_shrink_threshold=0.60,
        risk_composite_floor=0.40,
    )
